...
public class Ice_Dagger {
	...
	public Ice_Dagger(double x, double y, double ori, double mag, int lifetime, int amp) {
		this.x = x;
		this.y = y;
		this.init_x = x;
		this.init_y = y;
		...
		Main.archmage.ice_daggers.add(this);
		try {
			this.img = Main.r.rotate(ImageIO.read(new File("projectiles\\icedagger.png")), ori+45);
		}
		catch (IOException e) {
	...
	public void move() {
		...
		if(this.lifetime == 0) {
			Main.archmage.ice_daggers.remove(this);
		}
	}
}
